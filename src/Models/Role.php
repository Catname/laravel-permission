...
use Catname\Permission\Exceptions\RoleAlreadyExists;
use Catname\Permission\Exceptions\RoleDoesNotExist;
use Catname\Permission\Guard;
use Catname\Permission\PermissionRegistrar;
use Catname\Permission\Traits\HasPermissions;
...
class Role extends Model implements RoleContract
{
     ...
     *
     * @return \Catname\Permission\Contracts\Role|\Catname\Permission\Models\Role
     *
    ...
    public static function findOrCreate2(array $attributes = []): RoleContract
    {
        ...
        $role = static::findByParam(['name' => $attributes['name'], 'guard_name' => $attributes['guard_name']]);

        if (! $role) {
            return static::query()->create($attributes + (PermissionRegistrar::$teams ? [PermissionRegistrar::$teamsKey => app(PermissionRegistrar::class)->getPermissionsTeamId()] : []));
        }
